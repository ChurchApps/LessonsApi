...
export class AddOnPlaylistController extends LessonsBaseController {
  ...
  @httpPost("/")
  public async save(req: express.Request<{}, {}, AddOnPlaylist[]>, res: express.Response): Promise<interfaces.IHttpActionResult> {
    return this.actionWrapper(req, res, async (au) => {
      if (!au.checkAccess(Permissions.lessons.edit)) return this.json({}, 401);
      else {
        const promises: Promise<AddOnPlaylist>[] = [];
        req.body.forEach(addOnPlaylist => { addOnPlaylist.churchId = au.churchId; promises.push(this.repositories.addOnPlaylist.save(addOnPlaylist)); });
