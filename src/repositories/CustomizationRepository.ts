...
export class CustomizationRepository {
  ...
  public async create(customization: Customization) {
    ...
    const sql = "INSERT INTO customizations (id, churchId, venueId, contentType, contentId, action, actionContent) VALUES (?, ?, ?, ?, ?, ?, ?);";
    const params = [customization.id, customization.churchId, customization.venueId, customization.contentType, customization.contentId, customization.action, customization.actionContent];
    await DB.query(sql, params);
    return customization;
  }

  public async update(customization: Customization) {
    const sql = "UPDATE customizations SET venueId=?, contentType=?, contentId=?, action=?, actionContent=? WHERE id=? AND churchId=?";
    const params = [customization.venueId, customization.contentType, customization.contentId, customization.action, customization.actionContent, customization.id, customization.churchId];
    await DB.query(sql, params);
    return customization;
  }
