...
export class AddOnRepository {
  ...
  public async create(addOn: AddOn) {
    ...
    const sql = "INSERT INTO addOns (id, churchId, providerId, category, name, image) VALUES (?, ?, ?, ?, ?, ?);";
    const params = [addOn.id, addOn.churchId, addOn.providerId, addOn.category, addOn.name, addOn.image];
    await DB.query(sql, params);
    return addOn;
  }

  public async update(addOn: AddOn) {
    const sql = "UPDATE addOns SET providerId=?, category=?, name=?, image=? WHERE id=? AND churchId=?";
    const params = [addOn.providerId, addOn.category, addOn.name, addOn.image, addOn.id, addOn.churchId];
    await DB.query(sql, params);
    return addOn;
  }
