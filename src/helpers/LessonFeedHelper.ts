...
export class LessonFeedHelper {
  static async getExpandedLessonData(program: Program, study: Study, lesson: Lesson) {
    ...
    promises.push(
      Repositories.getCurrent()
        .externalVideo.loadPublicForLesson(lesson.id)
        .then(ev => (externalVideos = ev))
    );
  ...
  private static async getBundles(lessonId: string) {
    ...
    if (fileIds.length > 0) {
      ...
      bundles.forEach(b => {
        b.file = ArrayHelper.getOne(files, "id", b.fileId);
      });
    }
    return bundles;
  }
  ...
  private static async getAddOns(lessonId: string) {
    ...
    addOns.forEach(a => {
      a.file = ArrayHelper.getOne(files, "id", a.fileId);
    });
    return addOns;
  }
  ...
  private static convertVideoFile(video: ExternalVideo, download: boolean) {
    ...
    if (video.loopVideo) file.loop = true;
    else file.seconds = video.seconds;
    return file;
  }
